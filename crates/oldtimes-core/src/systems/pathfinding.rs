...
use crate::{
    components::{Pathfinding, Position, Tile},
    events::{MapChangedEvent, PathfindingRequestEvent},
...
mod tests {
    ...
    fn test_pathfinding_with_obstacles() {
        ...
        let path = path.unwrap();
        assert!(path.len() > 9); // Should be longer than direct path (which would be 9)
    }
