...
use crate::{
    ...
    events::{StartProductionEvent, ProductionCompletedEvent},
};
...
pub fn production_system(
    ...
    for (entity, building, mut producer, mut stockpile) in query.iter_mut() {
        ...
        if producer.production_progress >= 1.0 {
            complete_production(entity, &mut producer, &mut stockpile, recipe, &mut completed_events);
        }
    }
}
...
fn complete_production(
    ...
    log::debug!("Production completed: {} at building {:?}", recipe.name, entity);
}
