...
use crate::{CameraController, BuildingPlacer, DebugOverlay};
...
pub fn ui_input_system(
    ...
    if keyboard.just_pressed(KeyCode::F1) {
        ...
        log::info!("Debug overlay: {}", if debug_overlay.enabled { "ON" } else { "OFF" });
    }
    
    // Toggle pathfinding visualization
    if keyboard.just_pressed(KeyCode::F2) {
        debug_overlay.show_pathfinding = !debug_overlay.show_pathfinding;
        log::info!("Pathfinding debug: {}", if debug_overlay.show_pathfinding { "ON" } else { "OFF" });
    }
    
    // Toggle performance metrics
    if keyboard.just_pressed(KeyCode::F3) {
        debug_overlay.show_performance = !debug_overlay.show_performance;
        log::info!("Performance debug: {}", if debug_overlay.show_performance { "ON" } else { "OFF" });
    }
}