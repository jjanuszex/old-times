...
pub fn save_game_state(world: &mut World, filename: &str) -> Result<()> {
    ...
    for (entity, position, building, stockpile, worker, producer, tile, blocked, road) in
        query.iter(world)
    {
...
impl ReplayRecorder {
    ...
    pub fn record_event(&mut self, tick: u64, event_data: crate::events::ReplayEventData) {
        self.events
            .push(crate::events::ReplayEvent { tick, event_data });
    }
...
mod tests {
    ...
    fn test_save_load_game_state() {
        ...
        save_game_state(&mut world, filename).unwrap();
    ...
    fn test_replay_recording() {
        ...
        recorder.record_event(
            10,
            crate::events::ReplayEventData::PlaceBuilding(crate::events::PlaceBuildingEvent {
