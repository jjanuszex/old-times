...
impl EconomyAnalyzer {
    ...
    pub fn analyze_production_graph(&self) -> ProductionGraphAnalysis {
        ...
        for (recipe_id, recipe) in &self.config.recipes {
            ...
            for output in recipe.outputs.keys() {
                producers.entry(output.clone()).or_default().insert(recipe_id.clone());
            }
            
            // Track what each recipe depends on
            for input in recipe.inputs.keys() {
                dependencies.entry(recipe_id.clone()).or_default().insert(input.clone());
            }
        }
        ...
        analysis.resource_sources = self.config.recipes.iter()
            .filter(|(_, recipe)| recipe.inputs.is_empty())
            .map(|(id, _)| id.clone())
            .collect();
        
        // Find resource sinks (items that are only consumed, never produced)
        let all_inputs: HashSet<String> = self.config.recipes.values()
            .flat_map(|recipe| recipe.inputs.keys())
            .cloned()
            .collect();
        
        let all_outputs: HashSet<String> = self.config.recipes.values()
    ...
    pub fn calculate_flow_rates(&self, production_rates: &HashMap<String, f32>) -> HashMap<String, f32> {
    ...
    fn calculate_ratios_recursive(
        ...
        if visited.contains(item) {
            return Err(anyhow::anyhow!("Circular dependency detected for item: {}", item));
        }
        
        visited.insert(item.to_string());
        
        // Find recipes that produce this item
        let producers: Vec<_> = self.config.recipes.iter()
    ...
    fn dfs_cycle_detection(
        ...
        if let Some(deps) = dependencies.get(recipe_id) {
            for dep in deps {
                ...
                let dep_producers: Vec<_> = self.config.recipes.iter()
...
impl ProductionGraphAnalysis {
    ...
    pub fn is_resource_available(&self, resource: &str) -> bool {
        self.producers.contains_key(resource) || self.resource_sources.iter().any(|recipe_id| {
...
mod tests {
    use super::*;
    use crate::resources::{RecipeConfig, GameConfig};
    ...
    fn test_production_graph_analysis() {
        ...
        config.recipes.insert("make_planks".to_string(), RecipeConfig {
        ...
        config.recipes.insert("make_furniture".to_string(), RecipeConfig {
