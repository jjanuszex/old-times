...
impl DataLoader {
    ...
    pub fn load_from_directory<P: AsRef<Path>>(data_dir: P) -> Result<crate::resources::GameConfig> {
    ...
    fn load_buildings<P: AsRef<Path>>(&mut self, path: P) -> Result<()> {
        ...
        let buildings: HashMap<String, crate::resources::BuildingConfig> = toml::from_str(&content)?;
    ...
    fn load_mapgen_config<P: AsRef<Path>>(&self, path: P) -> Result<crate::resources::MapGenerationConfig> {
        let content = fs::read_to_string(path)?;
    ...
    fn validate_building(&self, building: &crate::resources::BuildingConfig) -> Result<()> {
        ...
        if building.construction_time <= 0.0 {
            return Err(anyhow::anyhow!("Building construction time must be positive"));
        }
