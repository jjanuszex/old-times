...
impl SpriteMetadataResource {
    ...
    pub fn load_from_file(path: &str) -> Result<SpriteMetadata> {
        ...
        log::info!(
            "Loaded sprite metadata: {} tiles, {} buildings, {} units",
            metadata.tiles.len(),
...
pub fn load_sprite_metadata_system(mut commands: Commands) {
...
pub fn hot_reload_sprite_metadata_system(mut metadata_resource: ResMut<SpriteMetadataResource>) {
...
fn load_sprite_metadata_resource() -> SpriteMetadataResource {
    ...
    match SpriteMetadataResource::load_from_file("assets/data/sprites.toml") {
        ...
        Err(e) => {
            log::warn!(
                "Failed to load sprite metadata: {}. Using fallback asset loading.",
                e
            );
        }
    }
...
"#;
        ...
        // Load metadata
        let metadata =
            SpriteMetadataResource::load_from_file(sprites_path.to_str().unwrap()).unwrap();
...
"#,
        ...
        let metadata =
            SpriteMetadataResource::load_from_file(sprites_path.to_str().unwrap()).unwrap();
        let atlas_map =
            SpriteMetadataResource::load_atlas_map(atlas_path.to_str().unwrap()).unwrap();
