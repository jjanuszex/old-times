use bevy::prelude::*;
use crate::{
    components::{Building, Position, Stockpile, Blocked},
    resources::{GameConfig, GameTick},
    events::{PlaceBuildingEvent, BuildingConstructedEvent},
};
...
pub fn building_placement_system(
    ...
    for event in events.read() {
        ...
        if is_position_occupied(&existing_buildings, event.position, building_config.size) {
            log::warn!("Cannot place building at {:?} - position occupied", event.position);
            continue;
        }
...
pub fn construction_system(
    ...
    for (entity, mut building, position) in query.iter_mut() {
        ...
        if building.construction_progress >= 1.0 {
            ...
            log::info!("Construction completed: {} at {:?}", building_config.name, position);
        }
    }
}
...
mod tests {
    ...
    fn test_position_occupation() {
        ...
        let query = world.query::<&Position, With<Building>>();
        
        // Test exact overlap
        assert!(is_position_occupied(&query, Position::new(5, 5), (1, 1)));
        
        // Test no overlap
        assert!(!is_position_occupied(&query, Position::new(10, 10), (1, 1)));
        
        // Test size overlap
        assert!(is_position_occupied(&query, Position::new(4, 4), (2, 2)));
    }
}