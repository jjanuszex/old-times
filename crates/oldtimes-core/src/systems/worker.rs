use crate::{
    components::{Building, Position, TaskPurpose, Worker, WorkerTask},
    events::{AssignWorkerEvent, PathfindingPriority, PathfindingRequestEvent, TaskCompletedEvent},
    resources::GameTick,
};
use bevy::prelude::*;
        ...
        if let Ok(mut worker) = workers.get_mut(event.worker) {
            ...
            if let Some(old_building) = worker.assigned_building {
                if let Ok(mut old_building_comp) = buildings.get_mut(old_building) {
                    old_building_comp.assigned_workers =
                        old_building_comp.assigned_workers.saturating_sub(1);
                }
            }
...
pub fn spawn_workers_system(mut commands: Commands, tick: Res<GameTick>) {
    // Spawn some initial workers at the start
    if tick.current == 1 {
        for i in 0..5 {
            commands.spawn((Position::new(10 + i, 10), Worker::new("worker".to_string())));
        }
